...
export class GeofenceMonitor {
  ...
  private async fetchGeofences(): Promise<void> {
    try {
      ...
      const response = await fetch(`${apiUrl}/api/public/geofences`)
  ...
  private async handleServerEvaluation(position: GeolocationPosition): Promise<void> {
    ...
    if (this.lastServerReportTime > 0 && timeSinceLastReport < MIN_REPORT_INTERVAL) {
      if (this.options.debug) {
        console.log(
          `[GeofenceMonitor] Rate limit: ${Math.ceil(
            (MIN_REPORT_INTERVAL - timeSinceLastReport) / 1000,
          )}s until next report allowed`,
        )
      }
      return
    }
        ...
        for (const event of response.events) {
          if (event.type === "enter") {
            ...
            if (!this.currentGeofences.has(event.geofence.id)) {
              if (this.options.debug) {
                console.log(
                  `[GeofenceMonitor] ✓ Emitting ENTER event for: ${event.geofence.name} (currently in ${this.currentGeofences.size} geofences)`,
                )
              }
            ...
            if (this.currentGeofences.has(event.geofence.id)) {
              if (this.options.debug) {
                console.log(
                  `[GeofenceMonitor] ✓ Emitting EXIT event for: ${event.geofence.name} (currently in ${this.currentGeofences.size} geofences)`,
                )
              }
  ...
  private async reportPositionToServer(report: PositionReport): Promise<PositionReportResponse> {
    const apiUrl = this.options.apiUrl.replace(/\/$/, "")
    const response = await fetch(`${apiUrl}/api/events/position`, {
      method: "POST",
      headers: {
